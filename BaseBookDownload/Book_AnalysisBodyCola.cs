    ...
    public class CoraBookNovelContent : BaseBookNovelContent, IFetchNovelContent
    {
        public bool AnalysisHtmlBookBody(IBaseMainWindow wndMain, BaseWndContextData datacontext, string strUrl, string strBody, bool bSilenceMode = false, DownloadStatus? status = null, int nMaxRetry = 0)
        {
            ...
            if (body != null)
            {
                ...
                {
                    FinishDocAnalyis(wndMain, datacontext, nextLink, header, content, bSilenceMode, status);
                    return true;
                }
                else
                {
        ...
        private void FinishDocAnalyis(IBaseMainWindow wndMain, BaseWndContextData datacontext, HtmlNode? nextLink, HtmlNode? header, HtmlNode? content, bool bSilenceMode, DownloadStatus? status = null)
        {
